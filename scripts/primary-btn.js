...
function createPrimaryButton({
  ...
  // Adicionar classes
  const baseClasses =
    "bg-primary text-[#333333] p-6 rounded-md inline-block transition-all duration-300 hover:bg-primary/90 hover:text-white hover:shadow-lg hover:text-xl transform hover:-translate-y-1 cursor-pointer";
  ...
  button.addEventListener("click", (e) => {
    ...
    if (useDialog) {
      ...
      if (typeof window.openFormDialog === "function") {
        ...
        console.error(
          "Primary Button: A função openFormDialog não está disponível. Certifique-se de que form-dialog.js está carregado."
        );
...
function renderPrimaryButton(containerId, props) {
  ...
  if (!container) {
    console.error(
      `Primary Button: Container com ID "${containerId}" não encontrado`
    );
    return;
  }
